...
def _prep_bytes(target: str, ext: str) -> tuple[bytes | None, str, str]:
    ...
    try:
        ...
        return None, "", "อ่านไฟล์รูปไม่สำเร็จครับ"

    try:
        from PIL import Image  # optional
    except ImportError:
        if ext in _CONVERTIBLE_EXTS:
            return None, "", f"ไฟล์ {ext} นี้ต้องมี Pillow ถึงจะอ่านได้ครับ (ลองแปลงเป็น .png ก่อน)"
        if len(raw) > _MAX_IMAGE_BYTES:
            return None, "", "ไฟล์รูปใหญ่เกินไปครับ (เกิน 12MB) ลองย่อขนาดก่อนนะครับ"
    ...
    try:
        ...
        return None, "", "เปิดไฟล์รูปนี้ไม่ได้ครับ (ไฟล์อาจเสียหรือไม่ใช่รูปจริง)"
    ...
    if len(data) > _MAX_IMAGE_BYTES:
        return None, "", "ไฟล์รูปใหญ่เกินไปครับ แม้ย่อแล้ว"
...
def analyze_image(path: str, question: str = "") -> str:
    ...
    if ext not in _IMAGE_EXTS and ext not in _CONVERTIBLE_EXTS:
        ...
        return f"ไฟล์นามสกุล {ext or '(ไม่มีนามสกุล)'} นี้ไม่ใช่รูปภาพที่ดูได้ครับ (รองรับ .png .jpg .jpeg .webp)"
    ...
    try:
        resp = _get_client().models.generate_content(
            ...
            config=types.GenerateContentConfig(
                # ตอบกลับเป็น tool result ที่ จาร์วิส เอาไปเรียบเรียงต่อ — ให้สั้น เป็นข้อความล้วน
                system_instruction=(
                    "ตอบเป็นภาษาไทย สั้นกระชับ ใช้คำลงท้ายแบบผู้ชาย (ครับ) "
        ...
        return f"ดูรูปไม่สำเร็จครับ ({type(e).__name__}) — ลองใหม่อีกทีนะครับ"

    files._log_activity("analyze_image", path, True)
    return text or "ดูรูปแล้วแต่ยังสรุปเป็นข้อความไม่ได้ครับ"
