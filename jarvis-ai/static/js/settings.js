...
/* ---------- โฟลเดอร์ที่อนุญาตให้ จาร์วิส อ่านไฟล์ได้: state จริงจาก server เท่านั้น (ไม่ใช่แค่จำ
...
function updateFolderBtnLabel(path) {
  if (path) {
    ...
    folderBtn.title = `จาร์วิส เข้าถึงไฟล์ได้แค่ในนี้: ${path} (กดเพื่อเปลี่ยน)`;
    folderBtn.classList.add('configured');
  } else {
    folderBtn.textContent = '📁 เลือกโฟลเดอร์';
    folderBtn.title = 'ยังไม่ได้ตั้งค่า — กดเพื่อเลือกโฟลเดอร์ที่ให้ จาร์วิส เข้าถึงไฟล์ได้';
