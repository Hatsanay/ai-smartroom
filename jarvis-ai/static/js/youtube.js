...
// ค่าเริ่มต้น แล้วสั่ง setVolume จริงถ้าทำได้ตอนนี้ (ถ้า จาร์วิส กำลังพูด = duck ค้างอยู่ ปล่อยให้
...
// (ที่ปกติไม่ต้องพูด "จาร์วิส" ซ้ำ) ไปก่อน กันเสียงร้อง/เนื้อเพลงถูกตีความเป็นคำสั่งมั่วๆ ดู
// เงื่อนไข !S.ytIsPlaying ใน wakeRecognition.onresult — ต้องพูด "จาร์วิส" นำทุกครั้งระหว่างเพลงเล่นอยู่
...
export function controlYoutube(action) {
  ...
  if (action === 'pause' && typeof S.ytPlayer.pauseVideo === 'function') {
    ...
    // ถ้า จาร์วิส กำลังพูดอยู่ (duck ค้างที่ YT_DUCK_VOLUME) อย่าเพิ่งใช้ค่าใหม่ทับตอนนี้ กันเสียงเพลง
    // ดังแทรกขึ้นมากลางที่ จาร์วิส พูดอยู่ — เก็บ ytVolume ไว้ก่อน พอ TTS จบ restoreYoutubeVolume()
...
// ย่อจอ YouTube ชั่วคราวตอนถูกเรียก จาร์วิส ระหว่างเต็มจอ (wave.js จับ rising edge ของ S.engagedActive
...
/* audio ducking: ลดเสียงเพลง YouTube ตอน จาร์วิส พูด (TTS) — เรียกจาก sound.js */
