"""ความจำถาวรของ จาร์วิส — ข้อเท็จจริงเกี่ยวกับผู้ใช้ (ชื่อคน, วันเกิด, ความชอบ, เป้าหมาย, กิจวัตร ฯลฯ)
...
def remember(fact: str, tag: str = "") -> str:
    ...
    fact = (fact or "").strip()
    if not fact:
        return "ยังไม่มีอะไรให้จำเลยครับ บอกมาได้เลย"
    db.execute(
        "INSERT INTO memory (text, tag, created_at) VALUES (?, ?, ?)",
        (fact, (tag or "").strip(), datetime.now().isoformat(timespec="seconds")),
    )
    return f"จำไว้แล้วครับ: {fact}"


def recall(query: str = "") -> str:
    """ค้นสิ่งที่จาร์วิสจำไว้เกี่ยวกับผู้ใช้ ใช้เมื่อผู้ใช้ถาม "เคยบอกอะไรไว้เรื่อง..." / "จำได้ไหมว่า..." หรือ
    ...
    if not rows:
        return "ยังไม่ได้จำอะไรไว้เลยครับ" if not query else f'ไม่มีบันทึกที่เกี่ยวกับ "{query}" ครับ'
...
def forget(query: str) -> str:
    ...
    query = (query or "").strip()
    if not query:
        return "บอกด้วยว่าจะให้ลืมเรื่องอะไรครับ"
    like = f"%{query}%"
    _id, n = db.execute("DELETE FROM memory WHERE text LIKE ? OR tag LIKE ?", (like, like))
    return f"ลบไป {n} รายการครับ" if n else f'ไม่เจอบันทึกที่ตรงกับ "{query}" ครับ'
...
def memory_preamble() -> str:
    ...
    return (
        "(ข้อมูลพื้นหลังที่จาร์วิสจำไว้เกี่ยวกับผู้ใช้ — ใช้ประกอบการตอบตามเหมาะสม "
