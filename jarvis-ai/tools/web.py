...
def search_web(query: str) -> str:
    ...
    try:
        ...
        return "ไม่พบผลการค้นหาสำหรับคำนี้ครับ (หรือระบบค้นหาอาจติดขัดชั่วคราว)"
...
def _url_is_safe(url: str) -> tuple[bool, str]:
    ...
    if p.scheme not in ("http", "https") or not p.hostname:
        return False, "ใส่ลิงก์ที่ขึ้นต้นด้วย http:// หรือ https:// นะครับ"
    try:
        infos = socket.getaddrinfo(p.hostname, p.port or (443 if p.scheme == "https" else 80))
    except socket.gaierror:
        return False, f'เปิดลิงก์ไม่ได้ครับ หาที่อยู่ของ "{p.hostname}" ไม่เจอ'
    for info in infos:
        ...
        if (
            ...
            return False, "ลิงก์นี้ชี้ไปที่อยู่ภายในเครือข่าย เปิดให้ไม่ได้ครับ"
    return True, ""
...
def read_url(url: str) -> str:
    ...
    try:
        ...
        html = None
    if not html:
        return "โหลดหน้าเว็บนี้ไม่สำเร็จครับ (เข้าไม่ได้ / ช้าเกินไป / ต้องล็อกอิน)"

    text = trafilatura.extract(
        html, include_comments=False, include_tables=True, favor_precision=True
    )
    if not text or not text.strip():
        return "ดึงเนื้อหาจากหน้านี้ไม่ได้ครับ (อาจเป็นหน้าที่ต้องรัน JavaScript หรือไม่มีบทความ)"
