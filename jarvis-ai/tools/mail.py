...
_NOT_SET = (
    "ยังไม่ได้ตั้งค่าอีเมลครับ ใส่ EMAIL_ADDRESS กับ EMAIL_APP_PASSWORD (Gmail App Password) "
    "ในไฟล์ .env แล้ว restart server ก่อนนะครับ"
)
    ...
    try:
        ...
        try:
            if kind == "inbox":
                ...
                if not seqs:
                    return "ไม่มีเมลใหม่ที่ยังไม่ได้อ่านครับ"
                total = len(seqs)
                pick = seqs[-max(1, n):][::-1]
            else:
                if count <= 0:
                    return f"ไม่มี{label}ครับ"
                total = count
        ...
        return f"เช็คเมลไม่สำเร็จครับ ({type(e).__name__}) — ลองใหม่อีกทีนะครับ"
...
def search_email(query: str, folder: str = "all", limit: int = 10) -> str:
    ...
    query = (query or "").strip()
    if not query:
        return "จะค้นเมลด้วยคำว่าอะไรครับ"
    ...
    try:
        ...
        try:
            ...
            if not seqs:
                ...
                return f'ไม่เจอเมล{where}ที่ตรงกับ "{query}" ครับ'
        ...
        return f"ค้นเมลไม่สำเร็จครับ ({type(e).__name__})"
...
def save_attachment(query: str, name: str = "", dest: str = "") -> str:
    ...
    if not files.get_allowed_folder():
        return "ยังไม่ได้ตั้งค่าโฟลเดอร์ปลายทางครับ กดปุ่ม 📁 เลือกโฟลเดอร์ในหน้าเว็บก่อนนะครับ"
    ...
    try:
        ...
        try:
            ...
            if seq is None:
                return "ไม่แน่ใจว่าจะเอาไฟล์แนบจากเมลฉบับไหน ลองเช็ค/ค้นเมลก่อนแล้วบอกลำดับหรือชื่อผู้ส่งครับ"
            _t, data = m.fetch(seq, "(RFC822)")
            msg = message_from_bytes(data[0][1])
        finally:
            m.logout()
    except Exception as e:
        return f"ดึงไฟล์แนบไม่สำเร็จครับ ({type(e).__name__})"
    ...
    if not saved and not skipped:
        return (
            f'ไม่เจอไฟล์แนบที่ชื่อมีคำว่า "{name}" ในเมลฉบับนี้ครับ'
            if want else "อีเมลฉบับนี้ไม่มีไฟล์แนบครับ"
        )
    out = []
    if saved:
...
def read_email(query: str, folder: str = "") -> str:
    ...
    try:
        ...
        try:
            ...
            if seq is None:
                return (
                    ...
                    "หรือบอกชื่อผู้ส่ง/ผู้รับ/หัวข้อครับ"
                )
            _t, data = m.fetch(seq, "(RFC822)")
            msg = message_from_bytes(data[0][1])
        ...
        return f"อ่านเมลไม่สำเร็จครับ ({type(e).__name__})"
...
# ต่อท้ายทุกเมลที่ JARVIS ส่ง (ผู้ใช้ขอ) — "ชื่อ" มาจาก EMAIL_SENDER_NAME ใน .env ถ้ามี ไม่งั้นใช้อีเมล
_SIG_MARKER = "J.A.R.V.I.S ผู้ช่วย AI ของคุณ"
...
def _send_preview(to: str, subject: str, body: str, att_line: str = "") -> str:
    ...
    return (
        ...
        + f"เนื้อหา:\n{b}\n"
        "— จบเนื้อหา —\n\n"
        'ถ้าถูกต้องแล้วบอก "ยืนยันส่ง" ครับ ถ้าจะแก้บอกได้เลย'
    )
...
def send_email(to: str, subject: str, body: str, attachments: str = "", confirm: bool = False) -> str:
    ...
    หมายเหตุ: ระบบจะต่อท้ายเนื้อหาด้วยลายเซ็น "นี่คือการส่งอีเมลจาก J.A.R.V.I.S ..." ให้อัตโนมัติทุกฉบับ
    ...
    to_n = _norm_to(to)
    if "@" not in to_n:
        return f'ที่อยู่ผู้รับ "{to}" ดูไม่ถูกต้องครับ'
    subj_n = (subject or "").strip()
    body_n = _with_signature(body, cfg)  # ต่อท้ายลายเซ็น JARVIS ก่อนพรีวิว/เทียบ/ส่ง — ผู้ใช้เห็นในพรีวิวด้วย
    ...
    if not confirm or not same or aerrs:
        if aerrs and confirm:
            note = "ไฟล์แนบยังมีปัญหา ส่งไม่ได้จนกว่าจะแก้ให้ครบนะครับ\n\n"
        elif confirm and p and not same:
            note = "ข้อมูลไม่ตรงกับที่ทวนไว้ ขอทวนใหม่อีกรอบนะครับ\n\n"
        else:
            note = ""
    ...
    try:
        ...
        return f"ส่งเมลไม่สำเร็จครับ ({type(e).__name__}) — เช็ค EMAIL_APP_PASSWORD กับการเชื่อมต่อดูนะครับ"
    _pending_send = None
    ...
    return f'ส่งเมลถึง {to_n} หัวข้อ "{em["Subject"]}"{extra} เรียบร้อยครับ'
