...
# เสียงปัจจุบันกลับไปด้วยเสมอ ให้ จาร์วิส มี context จริงมาอ้างอิงในบทสนทนาถัดไป ไม่ใช่เดามั่วๆ
_YT_VOLUME_STEP = 25
...
def open_youtube(query: str = "") -> str:
    ...
    try:
        ...
        return f'หาเพลง/วิดีโอ "{query}" ไม่สำเร็จครับ ระบบค้นหา YouTube อาจติดขัดชั่วคราว'

    if not results:
        _youtube_queue = []
        _youtube_queue_index = -1
        return f'ไม่พบผลลัพธ์สำหรับ "{query}" ใน YouTube ครับ'
    ...
    return f'เปิดเพลง "{first["title"]}" ให้แล้วครับ'
...
def control_youtube(action: str) -> str:
    ...
    if action == "next":
        if _youtube_queue_index < 0 or _youtube_queue_index + 1 >= len(_youtube_queue):
            return "หมดคิวเพลงที่ค้นไว้แล้วครับ ลองสั่งเปิดเพลงใหม่ได้เลยครับ"
        _youtube_queue_index += 1
        ...
        return f'เปลี่ยนเป็นเพลง "{track["title"]}" ให้แล้วครับ'
    ...
    if action in {"pause", "resume", "stop", "volume_up", "volume_down", "fullscreen", "exit_fullscreen"}:
        ...
        return {
            "pause": "หยุดเพลงไว้ชั่วคราวให้แล้วครับ",
            "resume": "เล่นเพลงต่อให้แล้วครับ",
            "stop": "ปิดเพลงให้แล้วครับ",
            "volume_up": f"เพิ่มเสียงเพลงให้แล้วครับ ตอนนี้อยู่ที่ {_youtube_volume}%",
            "volume_down": f"ลดเสียงเพลงให้แล้วครับ ตอนนี้อยู่ที่ {_youtube_volume}%",
            "fullscreen": "เปิดวิดีโอเต็มจอให้แล้วครับ (กด Esc เพื่อออก)",
            "exit_fullscreen": "ออกจากโหมดเต็มจอให้แล้วครับ",
        }[action]

    return "ไม่เข้าใจคำสั่งควบคุมเพลงนี้ครับ"
