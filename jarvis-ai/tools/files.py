...
# โฟลเดอร์โปรเจกต์ (jarvis-ai/) = แม่ของ tools/ ที่ไฟล์นี้อยู่ — logs/ กับ file_access_config.json
# ต้องอยู่ที่ระดับโปรเจกต์เหมือนเดิม ไม่ใช่ใน tools/ (ตอนแยก tools.py -> package `__file__` เลื่อนลงมา 1 ชั้น)
_PROJECT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# ไฟล์ log กิจกรรมเกี่ยวกับข้อมูลในเครื่อง — ผู้ใช้ขอไว้เป็นกลไกความโปร่งใส เพราะตอนนี้ จาร์วิส แก้ไข/
# ลบไฟล์ได้จริงแล้ว (ไม่ใช่แค่อ่าน) เก็บไว้ที่ jarvis-ai/logs/ (แยกจากโฟลเดอร์ที่ผู้ใช้เลือก ตั้งใจไม่
...
def list_files(subpath: str = "") -> str:
    """แสดงรายชื่อไฟล์/โฟลเดอร์ในโฟลเดอร์ที่ผู้ใช้อนุญาตให้ จาร์วิส เข้าถึงได้เท่านั้น (ตั้งค่าจากปุ่ม
    ...
    """
    if not _allowed_folder:
        return "ยังไม่ได้ตั้งค่าโฟลเดอร์ที่อนุญาตให้เข้าถึงเลยครับ กดปุ่ม 📁 เลือกโฟลเดอร์ มุมขวาบนของหน้าเว็บก่อนนะครับ"

    ok, target = _resolve_safe_path(subpath)
    if not ok:
        _log_activity("list_files", subpath, False, "outside allowed scope")
        return "ไม่สามารถเข้าถึงโฟลเดอร์นี้ได้ครับ อยู่นอกขอบเขตที่อนุญาตไว้"
    if not os.path.isdir(target):
        return f'"{subpath}" ไม่ใช่โฟลเดอร์ครับ'

    try:
        entries = sorted(os.listdir(target))
    except OSError:
        _log_activity("list_files", subpath, False, "OSError listing directory")
        return "เปิดโฟลเดอร์นี้ไม่สำเร็จครับ"

    _log_activity("list_files", subpath, True)
    if not entries:
        return "โฟลเดอร์นี้ว่างเปล่าครับ"
...
def read_file(path: str) -> str:
    """อ่านเนื้อหาไฟล์ข้อความ — จำกัดแค่ไฟล์ในโฟลเดอร์ที่ผู้ใช้อนุญาตให้ จาร์วิส เข้าถึงได้เท่านั้น
    ...
    if not ok:
        ...
        return "ไม่สามารถเข้าถึงไฟล์นี้ได้ครับ อยู่นอกขอบเขตที่อนุญาตไว้"
    if not os.path.isfile(target):
        return f'ไม่พบไฟล์ "{path}" ครับ'

    ext = os.path.splitext(target)[1].lower()
    if ext not in _ALLOWED_TEXT_EXTENSIONS:
        _log_activity("read_file", path, False, f"disallowed extension {ext}")
        return f"ไฟล์นามสกุล {ext or '(ไม่มีนามสกุล)'} นี้อ่านไม่ได้ครับ รองรับแค่ไฟล์ข้อความ (.txt, .md, .csv, .json, .py ฯลฯ)"
...
def create_folder(path: str) -> str:
    ...
    if not ok:
        ...
        return "ไม่สามารถสร้างโฟลเดอร์นี้ได้ครับ อยู่นอกขอบเขตที่อนุญาตไว้"

    try:
        os.makedirs(target, exist_ok=True)
    except OSError:
        _log_activity("create_folder", path, False, "OSError creating directory")
        return "สร้างโฟลเดอร์นี้ไม่สำเร็จครับ"

    _log_activity("create_folder", path, True)
    return f'สร้างโฟลเดอร์ "{path}" ให้แล้วครับ'


def write_file(path: str, content: str) -> str:
    """สร้างไฟล์ข้อความใหม่ หรือแก้ไข (เขียนทับทั้งไฟล์) ไฟล์ที่มีอยู่แล้ว — จำกัดแค่ในโฟลเดอร์ที่
    ผู้ใช้อนุญาตให้ จาร์วิส เข้าถึงได้เท่านั้น ใช้เมื่อผู้ใช้ขอให้สร้างไฟล์ใหม่หรือแก้ไขเนื้อหาไฟล์เดิม
    ...
    if not ok:
        ...
        return "ไม่สามารถเขียนไฟล์นี้ได้ครับ อยู่นอกขอบเขตที่อนุญาตไว้"

    ext = os.path.splitext(target)[1].lower()
    if ext not in _ALLOWED_TEXT_EXTENSIONS:
        _log_activity("write_file", path, False, f"disallowed extension {ext}")
        return f"ไฟล์นามสกุล {ext or '(ไม่มีนามสกุล)'} นี้เขียนไม่ได้ครับ รองรับแค่ไฟล์ข้อความ (.txt, .md, .csv, .json, .py ฯลฯ)"
    ...
    try:
        ...
        return "เขียนไฟล์นี้ไม่สำเร็จครับ"

    _log_activity("write_file", path, True, "overwritten" if existed else "created")
    return f'{"แก้ไข" if existed else "สร้าง"}ไฟล์ "{path}" ให้แล้วครับ'


def delete_path(path: str) -> str:
    """ลบไฟล์หรือโฟลเดอร์ (ถ้าเป็นโฟลเดอร์จะลบของข้างในทั้งหมดด้วย) — จำกัดแค่ในโฟลเดอร์ที่ผู้ใช้
    อนุญาตให้ จาร์วิส เข้าถึงได้เท่านั้น ใช้เมื่อผู้ใช้ขอให้ลบไฟล์/โฟลเดอร์ **อย่างชัดเจนเท่านั้น**
    ...
    if not ok:
        ...
        return "ไม่สามารถลบสิ่งนี้ได้ครับ อยู่นอกขอบเขตที่อนุญาตไว้"
    ...
    if target == os.path.realpath(_allowed_folder):
        ...
        return "ลบโฟลเดอร์หลักที่อนุญาตไว้ทั้งก้อนไม่ได้ครับ ต้องระบุไฟล์/โฟลเดอร์ย่อยข้างในแทนนะครับ"

    if not os.path.exists(target):
        return f'ไม่พบ "{path}" ครับ'
    ...
    try:
        ...
        return "ลบไม่สำเร็จครับ"

    _log_activity("delete_path", path, True)
    return f'ลบ "{path}" ให้แล้วครับ'
