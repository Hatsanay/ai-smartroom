...
def search_media(query: str, kind: str = "image") -> str:
    ...
    q = (query or "").strip()
    if not q:
        return "จะให้ค้นรูป/วิดีโอเรื่องอะไรครับ"
    k = _norm_kind(kind)
    try:
        if k == "video":
        ...
        return f'ค้นรูป "{q}" ไม่สำเร็จครับ (ระบบค้นหาอาจติดขัดชั่วคราว ลองใหม่อีกทีนะครับ)'
    except Exception:
        return f'ค้น{"วิดีโอ" if k == "video" else "รูป"}ไม่สำเร็จครับ ลองใหม่อีกทีนะครับ'

    items = [it for it in items if it["url"]]
    if not items:
        return f'ไม่เจอ{"วิดีโอ" if k == "video" else "รูป"}สำหรับ "{q}" ครับ'
...
def _local_gallery(rel: str):
    ...
    if not files.get_allowed_folder():
        return None, "ยังไม่ได้ตั้งค่าโฟลเดอร์ที่อนุญาตครับ"
    ok, tgt = files._resolve_safe_path(rel)
    if not ok:
        return None, f'"{rel}" อยู่นอกขอบเขตที่อนุญาตครับ'
    if os.path.isfile(tgt):
        if os.path.splitext(tgt)[1].lower() not in _MEDIA_EXTS:
            return None, "ไฟล์นี้ไม่ใช่รูป/วิดีโอที่เปิดดูได้ครับ"
    ...
    if os.path.isdir(tgt):
        ...
        return (items, "") if items else (None, f'ในโฟลเดอร์ "{rel}" ไม่มีรูป/วิดีโอครับ')
    return None, f'ไม่พบ "{rel}" ครับ'
...
def _download_video_async(url: str, target_dir: str, title: str) -> str:
    if len(_video_jobs) >= _MAX_VIDEO_JOBS:
        return f"กำลังโหลดวิดีโออยู่ {_MAX_VIDEO_JOBS} อันแล้วครับ รอให้เสร็จก่อนนะครับ"
    ok, msg = _url_is_safe(url)
    if not ok:
    ...
    def _work():
        try:
            ...
            with yt_dlp.YoutubeDL(opts) as ydl:
                ydl.download([url])
            notify.push(f"โหลดวิดีโอ '{title}' เสร็จแล้วครับ เก็บไว้ในโฟลเดอร์ที่เลือกไว้", "media")
            files._log_activity("download_media", title[:80], True, "video")
        except Exception as e:
            notify.push(f"โหลดวิดีโอ '{title}' ไม่สำเร็จครับ ({type(e).__name__})", "media")
            files._log_activity("download_media", title[:80], False, f"video {type(e).__name__}")
        finally:
            _video_jobs.discard(url)

    threading.Thread(target=_work, daemon=True, name="jarvis-vdl").start()
    return f"กำลังโหลดวิดีโอ '{title}' อยู่ครับ เดี๋ยวบอกตอนเสร็จ"
...
def download_media(ref: str, dest: str = "") -> str:
    ...
    """
    if not files.get_allowed_folder():
        return "ยังไม่ได้ตั้งค่าโฟลเดอร์ที่อนุญาตเลยครับ กดปุ่ม 📁 เลือกโฟลเดอร์ มุมขวาบนของหน้าเว็บก่อนนะครับ"
    dest = (dest or "").strip().strip("/\\")
    ok, target_dir = files._resolve_safe_path(dest)
    if not ok:
        return "โฟลเดอร์ปลายทางอยู่นอกขอบเขตที่อนุญาตครับ"
    try:
        os.makedirs(target_dir, exist_ok=True)
    except OSError:
        return "สร้างโฟลเดอร์ปลายทางไม่ได้ครับ"

    refs = _resolve_refs(ref)
    if not refs:
        return ("ไม่แน่ใจว่าจะโหลดอันไหน ลองค้นรูป/วิดีโอก่อนแล้วบอกลำดับ (เช่น \"โหลดอันที่ 2\") "
                "หรือส่งลิงก์มาก็ได้ครับ")
    ...
    if saved:
        ...
        parts.append(f"อยู่ที่ {where} — เปิดให้ดูแล้วครับ")
    return "\n".join(parts) if parts else "ไม่มีอะไรถูกบันทึกครับ"
...
def view_media(target: str = "", action: str = "open", seconds: int = 0) -> str:
    ...
    if act in ("close", "ปิด", "hide"):
        _state.pending_action = {"type": "hide_media"}
        return "ปิดหน้าต่างดูรูปแล้วครับ"

    if act in _STOP_WORDS:
        if not _viewer_list:
            return "ไม่มีสไลด์ที่เล่นอยู่ครับ"
        _show_viewer(_viewer_list, _viewer_index, slideshow=0)
        return "หยุดสไลด์แล้วครับ (ยังดูรูปนี้ค้างอยู่)"

    if act in ("next", "ถัดไป", "prev", "previous", "ก่อนหน้า", "back"):
        if not _viewer_list:
            return "ยังไม่มีรูป/วิดีโอเปิดอยู่ครับ"
    ...
    if t == "" or t.lower() in ("current", "ปัจจุบัน"):
        if _viewer_list:
            ...
            return "ยังไม่มีรูป/วิดีโอให้เปิดครับ ลองค้นก่อนนะครับ"
    elif t.lstrip("#").strip().isdigit():
        i = int(t.lstrip("#").strip()) - 1
        if not (0 <= i < len(_last_results)):
            return f"ไม่มีอันที่ {t} ในผลค้นล่าสุดครับ"
    ...
    if slide:
        return (f"เริ่มเล่นสไลด์แล้วครับ ({len(gallery)} รายการ เปลี่ยนทุก {slide} วิ วนไปเรื่อยๆ "
                f"บอก 'หยุด' เพื่อหยุด)")
    if len(gallery) > 1:
        return f"เปิดให้ดูแล้วครับ ({start + 1}/{len(gallery)} — ใช้ 'ถัดไป'/'ก่อนหน้า' เลื่อนได้)"
    return f"เปิด {gallery[0].get('name', '')} ให้ดูแล้วครับ"
