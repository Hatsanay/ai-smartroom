...
**ผู้ใช้ขอ:** จาร์วิส ค้นรูป/วิดีโอบนเว็บ → ดาวน์โหลดเก็บ → เปิดดูในหน้าเว็บ (ทั้งที่โหลดมาและจากผลค้นตรงๆ) → ปิดได้

**ตัดสินใจ (ถาม user):**
- เซฟที่ไหน: **จาร์วิส เลือกโฟลเดอร์ย่อยเองต่อครั้ง** (`dest` free-form, ว่าง = root) — บังคับ containment ในโฟลเดอร์ที่อนุญาตเสมอ
...
- return: "เจอ N \<รูป/วิดีโอ\>: 1. \<title\> (\<source\>) …" (จาร์วิส เล่าให้ผู้ใช้ + ผู้ใช้สั่งต่อ)
...
- `dest` = โฟลเดอร์ย่อยนับจากโฟลเดอร์ที่อนุญาต (จาร์วิส เลือกให้เหมาะกับเนื้อหา เช่น `"รูป/แมว"`, `"คลิป"`) · ว่าง = root
...
- **วิดีโอ (async — thread + notify):**
  ...
  - เสร็จ → `notify.push(f"โหลดวิดีโอ '{title}' เสร็จแล้ว เก็บที่ {relpath}", "media")` (→ `notify.js` พูด) · error → `notify.push("โหลดวิดีโอ … ไม่สำเร็จครับ")`
...
- `showMedia(data)` / `openViewer(list, i)` — `#mediaViewer` overlay:
  ...
  - แถบล่าง: title · ตัวนับ `i / n` · source domain · ปุ่ม **⬇ ดาวน์โหลด** (ยิง `form` ด้วยข้อความ "โหลดอันที่เปิดอยู่" → จาร์วิส เรียก `download_media("current")`) · ปุ่ม ✕
...
- `"current"` ใน `download_media`/`view_media` อ้าง state ฝั่ง server (`_viewer_index`) — ถ้าผู้ใช้คลิกเลื่อนรูปใน HUD เอง (ไม่ผ่าน จาร์วิส) server จะไม่รู้ index ล่าสุด → client ต้องแนบ index มาด้วยตอนกดปุ่ม ⬇ (แก้ใน Step 5: ปุ่ม ⬇ ส่งข้อความ "โหลดรูปที่ N")
