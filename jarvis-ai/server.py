...
def chat_endpoint(req: ChatRequest) -> ChatResponse:
    ...
    if quota_state["cooldown_until"] and now < quota_state["cooldown_until"]:
        ...
        return ChatResponse(reply=f"โควตา Gemini เต็มชั่วคราวครับ กรุณารออีก {remaining} วินาที")
    ...
    # แปะ fact ที่จำไว้ไว้หน้าข้อความทุกเทิร์น -> จาร์วิส เห็นข้อมูลผู้ใช้เสมอ (+ fact ที่เพิ่งเพิ่มด้วย
    ...
    try:
        ...
        if e.code == 429:
            ...
            return ChatResponse(
                reply=f"โควตา Gemini เต็มชั่วคราวครับ กรุณารออีกประมาณ {int(retry_seconds)} วินาที"
            )
        raise
        ...
        pop_pending_action()
        return ChatResponse(reply="ตอนนี้ Gemini กำลังมีผู้ใช้งานเยอะชั่วคราวครับ ลองถามใหม่อีกครั้งสักครู่นะครับ")
    finally:
...
def notifications_endpoint(response: Response) -> dict:
    # reminder ที่ scheduler thread เจอว่าถึงเวลาแล้ว — static/js/notify.js poll เอาไปแจ้ง + ให้ จาร์วิส พูด
    response.headers["Cache-Control"] = "no-store"
...
def browse_folder_endpoint() -> FolderStatus:
    ...
    try:
        selected = filedialog.askdirectory(title="เลือกโฟลเดอร์ที่ให้จาร์วิสเข้าถึงไฟล์ได้")
    finally:
        root.destroy()
