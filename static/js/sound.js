...
muteBtn.addEventListener('click', () => {
  muted = !muted;
  localStorage.setItem('jarvis_muted', muted ? '1' : '0');
  updateMuteBtn();
});
...
/* ---------- Text-to-Speech: จาร์วิส พูดตอบกลับด้วยเสียงจริงจาก server (PyThaiTTS/vachana) ----------
...
voiceSelect.addEventListener('change', () => {
  ...
  localStorage.setItem('jarvis_voice', selectedVoiceId);
});
...
// จาร์วิสเป็นเสียงผู้ชาย -> ค่าเริ่มต้นต้องเป็น 'vachana' ไม่ใช่ 'google'
// เพราะ Google Translate TTS มีเสียงไทยเดียว (เสียงผู้หญิง) เลือกเพศไม่ได้
// สลับไป google ได้จากเมนู "ตั้งค่า" แต่จะได้เสียงผู้หญิงแทน
let selectedEngine = localStorage.getItem('jarvis_tts_engine') || 'vachana';
...
engineSelect.addEventListener('change', () => {
  ...
  localStorage.setItem('jarvis_tts_engine', selectedEngine);
  applyEngineUI();
});
...
ttsAudio.addEventListener('play', () => {
  S.ttsSpeaking = true;
  pauseWakeListening(); // ปิดไมค์จริงๆ ตอน จาร์วิส พูด กัน STT หยิบเสียงตัวเองมาตีความ
  duckYoutubeVolume();
});
