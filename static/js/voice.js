...
if (SpeechRecognitionCtor) {
  ...
  recognition.onerror = (e) => {
    if (e.error === 'not-allowed' || e.error === 'service-not-allowed') {
      announceSystemNotice('ขออนุญาตใช้ไมโครโฟนก่อนนะครับ เช็กสิทธิ์ในเบราว์เซอร์ด้วยครับ');
    } else if (e.error !== 'no-speech' && e.error !== 'aborted') {
      announceSystemNotice(`ฟังเสียงไม่สำเร็จครับ (${e.error})`);
    }
  };
...
/* ---------- โหมดฟังตลอด: พูด "จาร์วิส" แล้วตามด้วยคำสั่งได้เลย ---------- */

// Google STT ถอดเสียง "จาร์วิส" ออกมาได้สารพัดแบบ (อังกฤษบ้าง ไทยหลายสะกดบ้าง สลับ จ/ย/ญ, ส/ด/ซ/ช,
...
const WAKE_WORD_PATTERNS = [
  // ---- โรมัน / อังกฤษ (Jarvis เป็นชื่อที่ STT รู้จัก โอกาสถอดเป็นอังกฤษสูง) ----
  // j/y/g/h/ch + a/e + (r) + v + i/y + (s/z/t/c) : jarvis javis jervis yarvis garvis charvis harvis jarviz jarvice
  /\b(?:ch|[jygh])[ae]r?\s?v[iy][sztc]?e?\b/i,
  /\bjar\s?vis\b/i,            // "jar vis" (STT ตัดเป็นสองคำ)
  // ---- ไทย: เต็มรูป มีตัว ร ("จาร์วิส / จารวิส / ยาร์วิส / ชาร์วิด / จาร์วิ") ----
  new RegExp('[จยญชฉ]' + T + '[าัะ]?' + T + '[รฤ]' + T + '์?' + T + '\s?ว' + T + '[ิีึ]?' + T + '[สซดทตถศษชล]?' + T + '์?'),
  // ---- ไทย: ตัว ร หาย (พบบ่อยสุด) ("จาวิส / จาวิด / ชาวิส / ยาวิซ") — บังคับต้องมีสระ ิ/ี + ตัวสะกด ----
  new RegExp('[จยญชฉ]' + T + '[าัะ]' + T + '\s?ว' + T + '[ิีึ]' + T + '[สซดทตถศษช]' + T + '์?'),
  // ---- ไทย: ตัว จ นำหล่น ("อาร์วิส / ราร์วิส / ทาร์วิส / ฮาร์วิส") ----
  /[อรลทหฮ]า?ร์?\s?ว[ิี][สซดท]/,
  // ---- ไทย: สั้นๆ เผื่อ STT ตัดหางคำ ("จาร์วิ / จาวิ") — lookbehind กันกิน space นำหน้า
  //          ไม่เอา "จาว" เดี่ยวๆ (เสี่ยงชน "ชาว/ข่าว") ต้องมีสระ ิ/ี เสมอ ----
  /(?<=^|\s)จ[่้๊๋]?า?ร?์?\s?ว[ิี](?=\s|$|[สซดทต])/,
];

// เรียกชื่อเฉยๆ ไม่มีคำสั่งตาม - ตอบรับสั้นๆ ด้วยเสียงจริง สุ่มสลับไว้กันซ้ำจำเจ
const WAKE_ACK_PHRASES = ['ครับผม มีอะไรให้ช่วยครับ', 'ว่าไงครับบอส', 'ฟังอยู่ครับ', 'ครับ พร้อมแล้วครับ'];
...
let lastInterim = '';         // interim ล่าสุด — เผื่อ session ตายก่อนได้ final ("จาร์วิส" คำเดียว)
...
// ช่วงคุยต่อเนื่องหลัง จาร์วิส ตอบ: ไม่ต้องพูด "จาร์วิส" ซ้ำใน 15 วิถัดไป
...
// "จาร์วิส" (ครอบคลุมช่วงกำลังประมวลผล + กำลังตอบด้วย) แล้วค่อยนับ 15 วิถอยหลังหลังตอบเสร็จจริงๆ
...
// สีเขียว + การโชว์แชทเหมือนกันหมด ส่วนข้อความ "ฟังต่อเนื่อง"/ไม่ต้องพูด "จาร์วิส" ซ้ำ (followUpActive)
...
// เรียกตอนเงียบเกิน 15 วิจริงๆ เท่านั้น -> ถือว่าเลิกคุยแล้ว กลับสีเดิม ต้องเรียก "จาร์วิส" ใหม่
export function expireFollowUpWindow() {
  ...
  // คุยกับ จาร์วิส จบแล้ว (พูดจบ + เงียบครบ 15 วิ ไม่มีถามต่อ) — ถ้าเมื่อกี้ย่อจอ YouTube ไว้เพราะโดนเรียก
...
// รอหลังพูดจบก่อนค่อยส่งคำสั่งจริง เผื่อผู้ใช้พูดต่อ (เดิมพอ recognition ตัดจบประโยคแรก
// ทันทีตอนหยุดพูดแป๊บนึงคิดคำ ระบบส่งคำสั่งที่ยังพูดไม่จบไปเลย) ทุกครั้งที่ได้ยินเสียงเพิ่มระหว่างรอ
// จะเอามาต่อท้ายแล้วรีเซ็ตนาฬิกาใหม่ จนกว่าจะเงียบจริงๆ ถึงส่งเป็นข้อความเดียว
// 2000 = ผู้ใช้ปรับลงจาก 5000 (2026-09-02) เพราะ 5 วิรู้สึกช้าเกินไปทุกคำสั่ง
const COMMAND_DEBOUNCE_MS = 2000;
let pendingCommandText = '';
let commandDebounceTimer = null;
let isAccumulatingCommand = false; // true ระหว่างรอ debounce — ให้พูดต่อได้โดยไม่ต้องพูด "จาร์วิส" ซ้ำ
...
// ปิดไมค์จริงๆ ตอน จาร์วิส พูด (ไม่ใช่แค่เช็ค flag) กัน STT หยิบเสียงตัวเองมาตีความ
...
function runWakeRecognition() {
  ...
  w.continuous = true;
  w.maxAlternatives = 3;     // Google มัก transcribe "จาร์วิส" เป็นตัวเลือก #2 ตอน #1 เพี้ยน
  ...
  w.onresult = (e) => {
    ...
    // ระหว่างคุยต่อเนื่อง / รอ debounce 5 วิ -> พูดอะไรมาก็เป็นส่วนของคำสั่งเดิม (ไม่ต้องพูด "จาร์วิส" ซ้ำ)
    // ยกเว้นตอนเพลง YouTube เล่นอยู่ (state จริงจาก YT.Player) -> บังคับต้องมี "จาร์วิส" นำเสมอ
  ...
  w.onerror = (e) => {
    ...
    if (e.error === 'not-allowed' || e.error === 'service-not-allowed') {
      stopWakeMode();
      announceSystemNotice('ขออนุญาตใช้ไมโครโฟนก่อนนะครับ ถึงจะเปิดโหมดฟังตลอดได้');
    }
  ...
  w.onend = () => {
    ...
    if (lastInterim && !S.ttsSpeaking) {
      if (utterWakeSeen && !utterHandled) {
        handleWakeFinal(lastInterim, null);            // "จาร์วิส [คำสั่ง]" คำเดียวสั้นๆ
