...
export function renderWaveRing() {
  ...
  // สีเขียว = S.engagedActive: เริ่มทันทีที่ได้ยินคำว่า "จาร์วิส" (ครอบคลุมช่วงประมวลผล+ตอบด้วย)
  // แล้วค้างไว้จนกว่าจะเงียบครบ 15 วิหลังตอบเสร็จจริงๆ ถึงจะกลับสีเดิม (ต้องเรียก "จาร์วิส" ใหม่)
  coreWrap.classList.toggle('mic-listening', S.engagedActive);
  // แชท: โชว์เฉพาะตอนกำลังเรียก/ใช้งาน จาร์วิส อยู่จริง (S.engagedActive) ส่วน :hover คุมเองด้วย CSS แล้ว
  log.classList.toggle('chat-active', S.engagedActive);

  // YouTube เต็มจออยู่แล้วผู้ใช้เพิ่งเรียก จาร์วิส (S.engagedActive ขึ้นขอบ) -> ย่อจอชั่วคราวให้เห็น HUD หลัก
