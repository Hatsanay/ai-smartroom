...
form.addEventListener('submit', async (e) => {
  ...
  S.engagedActive = true; // กำลังใช้งาน จาร์วิส อยู่จริง (พิมพ์/กดพูด/สั่งผ่าน wake mode) โชว์แชท+เขียวไว้ก่อน
  addLine('you', message);
  input.value = '';
  setThinking(true);
  sendBeep();
  ...
  try {
    ...
    addLine('jarvis', `เชื่อมต่อไม่ได้: ${err.message}`);
