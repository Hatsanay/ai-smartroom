# Plan 03 — ต่อ จัสมิน เข้ากับเครื่องพิมพ์ 3 มิติ Bambu Lab

> **สถานะ: ยังไม่เริ่ม — รอเครื่อง** · **รุ่นที่ผู้ใช้เลือกแล้ว = Bambu Lab P2S Combo (P2S + AMS 2 Pro)**
> เป้าหมายผู้ใช้: *"ผมมีหน้าที่แค่ออกแบบโมเดล แล้ววางไฟล์ — ที่เหลือ จัสมิน จัดการ"*
...
3. ลง **Bambu Studio** บนเครื่องที่รัน server → **export preset JSON 3 ไฟล์** (machine / process / filament)
   เก็บไว้ที่ `jusmin-ai/printer_profiles/` (gitignore — เป็นค่าเฉพาะเครื่อง)
...
| `printer_camera(question="")` | ดึงเฟรม → เซฟลงโฟลเดอร์ที่อนุญาต → ป้อน **`analyze_image()` ที่มีอยู่แล้ว** | "จัสมิน ดูเครื่องพิมพ์ให้หน่อย" |
...
| `printer_filaments()` | อ่าน AMS: ช่องไหนมีอะไร สีอะไร เหลือเท่าไร ความชื้น | ให้ จัสมิน เลือกไส้ให้เองได้ |
...
subscribe MQTT ค้างไว้ใน scheduler thread → event → `notify.push()` → **จัสมิน พูดเอง**
...
- **TPU/ไส้ยืดหยุ่นใส่ AMS ไม่ได้** → ต้องแขวนม้วนข้างเครื่องเอง จัสมิน เลือกให้อัตโนมัติไม่ได้
- **ห้องพิมพ์ไม่มีฮีตเตอร์** (P2S) → PPS-CF / PEEK ทำไม่ได้ ไม่ว่าจะสั่งยังไง
- ~~A1 กล้องช้า~~ **ไม่เกี่ยวแล้ว** — P2S กล้อง 1080p@30fps ฟีเจอร์ "ให้ จัสมิน ดูงานพิมพ์" ได้ผลเต็มที่
